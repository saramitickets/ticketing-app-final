...
// --- 5. CALLBACK ROUTE (REFINED FOR V15.2) ---
app.post('/api/payment-callback', async (req, res) => {
    console.log("📥 [LOG] Callback received from Payment Gateway");
    
    let rawData = req.body;
    
    // Handle potential stringified results within the body
    const results = (typeof rawData.results === 'string') ? JSON.parse(rawData.results) : rawData.results;
    
    // Exact mapping for the gateway's merchantTxnId and other common variants
    const mReqId = results?.merchantTxnId || results?.MerchantRequestID || results?.transactionId || rawData.transactionId;
    const statusCode = rawData.statusCode || results?.statusCode || results?.ResultCode;

    // Safety Check for Transaction ID
    if (!mReqId) {
        console.error("❌ [LOG] CALLBACK DATA ERROR: Transaction ID missing in payload.");
    ...
    try {
        ...
        if (querySnapshot.empty) {
            console.error(`⚠️ [LOG] Callback Warning: No order found in database for Ref: ${mReqId}`);
        ...
        // Check for success (200, 0, or "SUCCESS")
        const isSuccess = (statusCode == 200 || statusCode == 0 || statusCode === 'SUCCESS');
        ...
        if (isSuccess) {
            ...
            // Log as a user cancellation/decline
            const reason = rawData.message || results?.message || results?.ResultDesc || "Declined/Cancelled";
            console.log(`🛑 [LOG] USER CANCELLED: Order ${orderId}. Reason: ${reason}`);
            await orderRef.update({ 
                status: 'CANCELLED', 
                updatedAt: admin.firestore.FieldValue.serverTimestamp(), 
                cancelReason: reason 
